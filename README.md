...
Course project for ESE5971 focused on building a medical imaging recommendation system that can help learners, clinicians, and researchers find the most relevant educational content, references, and study materials for a given topic or question.
...
## 6. My Recommended Baseline Strategy

If we want a clean and defensible course-project roadmap, my recommendation is:

### Recommended baseline stack
1. **TF-IDF + cosine similarity** as the simplest sanity-check baseline
2. **BM25** as the main lexical baseline
3. **Embedding retrieval** as the main semantic baseline
4. **Hybrid BM25 + embedding** as the strongest practical baseline before re-ranking

### Why this is the best roadmap
- It is incremental and easy to explain in a report
- Each stage has a clear purpose
- We can show measurable improvement from one stage to the next
- It balances engineering feasibility and research value
- It creates a strong story for model selection

### What I would call the official baseline model
If the report needs **one baseline model**, I would choose:

> **BM25 over cleaned and chunked PDF text**

Reason:
- It is standard
- It is strong enough to be meaningful
- It is interpretable
- It fits the current state of the repository, which already has chunked text generation

### What I would call the strongest early-system baseline
If we are allowed to use a stronger “practical baseline,” I would choose:

> **Hybrid retrieval: BM25 + embedding similarity over cleaned chunks**

This is likely the best balance of performance and implementation effort for the current project scope.

---

## 7. Evaluation Plan

To compare model choices fairly, we should define a small labeled evaluation set.

### Suggested evaluation dataset
Create a set of queries such as:
- “Fourier transform in medical imaging”
- “What is convolution?”
- “Morphological operators explanation”
- “Difference between correlation and convolution”
- “Noise reduction in imaging systems”
...
A new model should only be considered better if it clearly improves top-k relevance over BM25 and remains understandable enough for the course project scope.

---

## 8. Near-Term Project Roadmap

### Phase A: Finish data pipeline validation
- Run the PDF pipeline end to end
- Inspect cleaned pages and chunk outputs
- Tune chunk size thresholds if needed
- Add preprocessing statistics to the report

### Phase B: Build retrieval baselines
- Implement TF-IDF baseline
- Implement BM25 baseline
- Create a small labeled query set
- Compare retrieval quality

### Phase C: Add semantic retrieval
- Generate embeddings for chunk text
- Build vector retrieval
- Compare against BM25

### Phase D: Build a hybrid system
- Combine lexical and semantic scores
- Tune weights on validation queries
- Report gains and failure cases

### Phase E: Optional advanced extension
- Add re-ranking
- Add personalization
- Add web/video sources
- Extend to multimodal recommendation
...
At the current stage, the strongest implemented part of the project is the **PDF preprocessing pipeline**, which extracts text, cleans pages, and creates structured chunks. The next major milestone should be building and evaluating the first retrieval baseline, with **BM25 as the official baseline model** and **hybrid retrieval as the most promising practical early-system baseline**.
